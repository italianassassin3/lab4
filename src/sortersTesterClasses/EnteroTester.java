...
public class EnteroTester {
	...
	public static void main(String[] args) { 
		...
		sortersList.add(new CombSortSorter<Entero>());
		sortersList.add(new CocktailSortSorter<Entero>());


		test("Sorting Using Default Comparator<Entero>", null); 
		//test("Sorting Using IntegerComparator1", new EnteroComparator1()); 
		//test("Sorting Using IntegerComparator2", new EnteroComparator2()); 
	}
	...
	private static Entero[] randomValues(int i) {
		int[] a = new int[i]; 
		Entero[] tmp = new Entero[i];
		for (int j=0; j<i; j++) {
			a[j] = rnd.nextInt(100); 
			Entero entero = new Entero(a[j]);
			tmp[j] = entero;
